...
| 项目 | 实测值 |
|---|---|
| 监控程序自身 CPU | 75 秒内累计 0.19 秒，约占全部核心的 **0.01%** |
| 监控程序内存 | 常驻约 **16.5 MB** |
| 每轮检查的瞬时开销 | 启动无头 Edge + node，峰值约 **0.9 GB**，持续 **7~15 秒**，之后全部释放 |
| 检查频率 | 约每 1 分钟一轮（本机）/ 每 10 分钟一轮（云端） |

结论：空闲时几乎无感；真正的开销是每分钟一次的浏览器瞬时启动（内存峰值 + 少量磁盘 IO）。

## 十、不要再监控时怎么彻底停掉

监控由 Windows 计划任务 `PuraXViewMonitorGuard` 托管（每 5 分钟检查一次、进程掉了自动拉起），
**已设置 2026/10/18 自动到期**，到期后不会再启动。此外提供三种手动方式：

| 目的 | 操作 |
|---|---|
| 彻底清除（推荐） | 双击 `取消监控托管.cmd`：删除计划任务 + 停止监控进程 |
| 只停当前进程 | 双击 `stop_monitor.cmd`（但计划任务会在 5 分钟内重新拉起） |
| 临时暂停两周 | 命令行执行 `schtasks /Change /TN "PuraXViewMonitorGuard" /DISABLE`，恢复用 `/ENABLE` |

> 注意：清除本机监控不会影响云端检查（GitHub Actions 由 cron-job.org 触发），
> 要一并停掉需在 cron-job.org 后台停用/删除该定时任务。
